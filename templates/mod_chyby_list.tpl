...
<tr>
	...
	<select name='order' onChange='document.f.submit();'>
	    <option value='pridano'{if $order=='pridano'} selected{/if}>Datum přidání</option>
	...
	<select name='desc' onChange='document.f.submit();'>
	    <option value=''{if $desc==''} selected{/if}>Vzestupně</option>
	    <option value='1'{if $desc=='1'} selected="selected"{/if}>Sestupně</option>
	</select>
    </td>
</tr><tr>
    <td>Stav:</td>
	...
	<select name='filter' onChange='document.f.submit();'>
            <option value=''{if $filter==''} selected{/if}>Všechny</option>
	    <option value='open'{if $filter=='open'} selected="selected"{/if}>Otevřené</option>
