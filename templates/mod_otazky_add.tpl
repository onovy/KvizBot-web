<h2>Přidání otázky</h2>

{include file="../onovyPHPlib/templates/message.tpl"}

<form method='POST'>
<input type='hidden' name='w' value='add2'>
<table border='0'>
<tr>
    <td>Otázka:</td><td><input type='text' name='otazka' maxlength='100' size='70' value='{$otazka|default:''}'></td>
</tr><tr>
    <td>Odpoveď:</td><td><input type='text' name='odpoved' maxlength='100' size='70' value='{$odpoved|default:''}'></td>
</tr><tr>
    <td>Téma:</td>
    <td>
	<select name='tema'>
	    <option value='0'>- vyberte -</option>
	{section name=sec1 loop=$temata}
	    <option value='{$temata[sec1]->id}'{if $tema|default:0 == $temata[sec1]->id} selected{/if}>{$temata[sec1]->nazev|escape}</option>
	{/section}
	</select>
    </td>
{if $auth->perm_a|default:false}
</tr><tr>
    <td>Schválení:</td>
    <td>
	<select name='schvaleni'>
	    <option value='0'>- nikdo -</option>
	{section name=sec1 loop=$schvaleni}
	    <option value='{$schvaleni[sec1]->id}'>{$schvaleni[sec1]->nick|escape}</option>
	{/section}
	</select>
    </td>
{/if}
</tr><tr>
    <td></td><td><input type='submit' value='Přidat'></td>
</tr><tr>
    <td colspan=2>Před přidáním otázky do Kvízu si prosím přečtěte <a href='licence-otazek.htm' target='_blank'>licencování</a></td>
</tr>
</table>
</form>

<hr />

<h3>Pravidla</h3>
{$pravidla}
