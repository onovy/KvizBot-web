...
if ($w=='info') {
    ...
    if (empty($added)) $added='Dříve než 26. 9. 2005';
...
if ($w=='move' && ($auth->perm_a ?? null)) {
    ...
    $done = false;
    if ($fa['id']) {
	$smarty->assign('error', 'Cílový nick již existuje, změnu není možné provést!');	
    } else {
...
if (!db_table_exists($table_month)) {
    show_error('Skóre pro tento měsíc neexistuje');
}
