...
function error_notnumeric($var) {
    show_error('Proměnná \''.$var.'\' musí být číslo!');
}

function error_notchar($var) {
    show_error('Proměnná \''.$var.'\' musí být jeden znak!');
}

function error_notarray($var,$array) {
    $error='Vstupní proměnná "'. HE($var) . '" musí nabývat jednu z hodnot: ';
    $first=1;
...
function error_notperm() {
    show_error('Nemáte dostatečná práva!');
}
